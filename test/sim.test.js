...
const fresh=(seed=1)=>{const S=createSim(seed);const inj=(L,R,lt=0,rt=0,step=1)=>{S.inject({p:L,q:[0,1,0,0],t:lt,g:0},{p:R,q:[0,1,0,0],t:rt,g:0},H);S.step(step)};
...
t('A1 rope: violent hands (6 m/s walks, teleports, crossing) at 72/90/120 Hz input → no NaN, bounded stretch and speed',()=>{
  for(const hz of[72,90,120]){const S=createSim(3);let L=[-.3,1.2,.3],R=[.3,1.2,.3],r=1;const rnd=()=>(r=r*16807%2147483647)/2147483647;
    let acc=0,steps=0;for(let f=0;f<hz*30;f++){acc+=1/hz;
      ...
      while(acc>=DT){S.inject({p:L,q:[0,1,0,0],t:lt,g:0},{p:R,q:[0,1,0,0],t:rt,g:0},H);S.step();acc-=DT;steps++;
...
t('A5 determinism: same seed + same input stream → identical hashState every 90 steps across two runs; different seed differs',()=>{
  ...
  const S=createSim(1);const hs=[];a.rec.forEach((f,i)=>{S.inject({p:f[0],q:f[1],t:f[2],g:f[3]},{p:f[4],q:f[5],t:f[6],g:f[7]},{p:f[8],q:f[9]});S.step();S.drain();if(i%90==0)hs.push(S.hashState())});eq(hs.join(),a.hs.join(),'replay mismatch');
