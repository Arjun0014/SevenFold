...
export function auInit(){if(auC)return;try{auC=new AudioContext;auG=auC.createGain();auG.connect(auC.destination);auHum=auO('sine',110,0)}catch(e){}}
export function auSync(S,ev,mute){if(!auC)return;auG.gain.value=mute?0:.4;if(auC.state=='suspended')auC.resume();
  auHum[1].gain.value=S._wp?0:S._ten*.012;auHum[0].frequency.value=110+S._ten*220;const tt=S._ten>=.7&&!S._wp;if(tt&&!auTaut)ev=ev.concat({k:'forged',b:3});auTaut=tt;
