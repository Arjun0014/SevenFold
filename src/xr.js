...
export const xrS={on:0,sup:0,ht:0,lo:0,h:[]}; // h: per controller {g: grip Object3D, hand, on, t (select), q (squeeze), src}
export async function xrInit(renderer,btn,onStart,onEnd){
  ...
  btn.onclick=async()=>{btn.hidden=true;onStart();if(!xrS.sup)return;
    try{const s=await navigator.xr.requestSession('immersive-vr',{optionalFeatures:['local-floor','bounded-floor','hand-tracking']});
      ...
      try{renderer.xr.setReferenceSpaceType('local-floor');await renderer.xr.setSession(s)}
      catch(e){renderer.xr.setReferenceSpaceType('local');await renderer.xr.setSession(s);xrS.lo=1} // no local-floor: lift by 1.6 m
