...
mB.id='b';mH.id='h';mH.textContent='LMB/RMB triggers · B arch · V grips · Space/G/N sigils · WASD/QE hands · R · M';
...
if(!R){mB.remove();mH.remove();mU.textContent=T&&T.WebGLRenderer?'Sevenfold needs WebGL.':'Sevenfold could not load Three.js from play.js13kgames.com. Check the connection and reload.'}
  ...
  const SF=window.SF={manual:0,rec:0,rd:rdM,R,
    ...
    state:()=>({wave:sim._wave,ws:sim._ws,mode:sim._md,light:sim._light,xr:xrS.on,calls:R.info.render.calls,tris:R.info.render.triangles,events:evLog.slice(-300),text:rdM.text,mute})};
  ...
  R.setAnimationLoop(t=>{
    ...
    const ev=sim.drain();for(const e of ev){evLog.push(e.k);const h={hit:.5,crack:.5,catch:.5,res:.9,block:.9,hurt:2}[e.k];if(h)xrPulse(h,h*100);if(e.k=='over'||e.k=='dawn')save()}
